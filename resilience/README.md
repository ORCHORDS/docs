---
title: "Resilience"
owner: "Operations Lead"
status: "approved"
classification: "public"
last-reviewed: "2026-08-23"
review-cycle: "90 days"
next-review: "2026-11-21"
---

# Resilience

Continuity, recovery, crisis coordination, dependency mapping, workforce resilience, and resilience testing.

## Documents

- [Business Impact Analysis](./BUSINESS_IMPACT_ANALYSIS.md)
- [Disaster Recovery Policy](./DISASTER_RECOVERY_POLICY.md)
- [Recovery Objective Recertification](./RECOVERY_OBJECTIVE_RECERTIFICATION.md)
- [Dependency Resilience](./DEPENDENCY_RESILIENCE.md)
- [Recovery Dependency Mapping](./RECOVERY_DEPENDENCY_MAPPING.md)
- [Alternate Dependency Strategy](./ALTERNATE_DEPENDENCY_STRATEGY.md)
- [Failover Assumption Review](./FAILOVER_ASSUMPTION_REVIEW.md)
- [People Dependency Resilience](./PEOPLE_DEPENDENCY_RESILIENCE.md)
- [Workforce Disruption Continuity](./WORKFORCE_DISRUPTION_CONTINUITY.md)
- [Emergency Operations](./EMERGENCY_OPERATIONS.md)
- [Crisis Role Recertification](./CRISIS_ROLE_RECERTIFICATION.md)
- [Crisis Communications](./CRISIS_COMMUNICATIONS.md)
- [Crisis Decision Logging](./CRISIS_DECISION_LOGGING.md)
- [Recovery Communications Governance](./RECOVERY_COMMUNICATIONS.md)
- [Recovery Exercise Evidence](./RECOVERY_EXERCISE_EVIDENCE.md)
- [Resilience Improvement Tracking](./RESILIENCE_IMPROVEMENT_TRACKING.md)
- [Resilience Exception Aging](./RESILIENCE_EXCEPTION_AGING.md)
- [Resilience Testing](./RESILIENCE_TESTING.md)
- [Tabletop Exercises](./TABLETOP_EXERCISES.md)

Recovery objectives are risk decisions and must be supported by business impact and tested capability.
