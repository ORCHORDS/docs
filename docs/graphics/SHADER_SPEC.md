...
This document specifies the shader system in Beetle Studio -- how shaders are organized, what parameters they expose, and how to add a new shader to the rendering pipeline. Per **ISO/IEC 12207:2017 section 6.1**, internal interfaces must be precisely specified so that subsystems (effects, rendering, color) can be developed independently.
