# feature-cookbook-feature-isolation
...
Deploys are weekly. Each deploy fixes one bug and breaks
two.
...
- **Single DB, separate schemas:** One DB, multiple
  schemas (e.g. `auth.users`, `billing.charges`)
...
For the boundary, the ACL (anti-corruption layer):
```ts
...
The ACL prevents one context's model from leaking.

## The "events" pattern

For cross-context communication, use events:
```ts
...
For code sharing, use a monorepo:
```
/packages
  /auth
  /billing
...
Each service has its own DB + deploy.

For most apps, **monorepo with modules** is enough; you
don't need full microservices.

## The "synchronous vs asynchronous" choice

For cross-context communication:
- **Synchronous (HTTP):** For real-time needs (e.g. user
  validation)
- **Asynchronous (queue):** For non-blocking (e.g.
  email notification)
...
For safe rollout, use feature flags:
```ts
...
- **The "no context boundary" anti-pattern.** One big
  monolith.
- **The "tight coupling" anti-pattern.** A change in A
  breaks B.
- **The "shared tables" anti-pattern.** A schema change
  affects multiple contexts.
- **The "cross-context transactions" anti-pattern.** Use
  saga.
