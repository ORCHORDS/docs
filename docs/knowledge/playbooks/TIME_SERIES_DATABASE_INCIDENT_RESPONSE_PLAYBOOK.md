# Time-Series Database Incident Response Playbook

## Purpose

Respond to security and reliability incidents affecting time-series databases (TimescaleDB, InfluxDB, QuestDB) and time-series ingest / query pipelines. The playbook aligns with [Time-Series Query Performance Governance](../standards/TIME_SERIES_QUERY_PERFORMANCE_GOVERNANCE.md), [Time-Series Database Architecture Governance](../standards/TIME_SERIES_DATABASE_ARCHITECTURE_GOVERNANCE.md), [Time-Series Retention & Compression Governance](../standards/TIME_SERIES_RETENTION_COMPRESSION_GOVERNANCE.md), and the standard security incident response process.

## Audience

On-call engineers, security responders, workload owners, Time-Series Platform engineers.

## Pre-conditions

1. The standard incident severity levels and the on-call rotation are documented.
2. The time-series database reference card is current (`TIMESCALEDB_VERSION_GOVERNANCE.md`, `INFLUXDB_VERSION_GOVERNANCE.md`, `QUESTDB_VERSION_GOVERNANCE.md`).
...
5. The query-latency SLO alert, the continuous-aggregate lag alert, the tenant-scope alert, and the compression-ratio alert are wired to the on-call paging rotation.

## Procedure

1. **Detect and triage.** Identify the incident class: unbounded time-range scan, high-cardinality tag filter DoS, SQL / InfluxQL / QuestDB SQL injection, cache poisoning, cross-tenant query, index bypass, query-latency SLO breach, ingest backlog breach, or availability outage. Assign severity using the standard matrix.
2. **Contain.** Reject malicious queries via the access layer; revoke the implicated service-account credentials; freeze the ingest pipeline if the incident involves source ingestion; isolate the affected cluster or schema.
3. **Eradicate.** Rotate credentials, rebuild the schema from a known-good snapshot, and remove malicious ingest agents or queries. Confirm the threat is no longer present before recovery.
4. **Recover.** Re-enable queries and ingest; restore the workload to the previous query-latency SLO; verify tenant scoping, lineage integrity, index usage, and continuous-aggregate lag before lifting the freeze.
...
6. **Investigate.** Reconstruct the timeline from access logs, audit logs, and OTLP traces; identify the affected rows via the lineage record; quantify the exposure (tenant, row count, data class).
...
10. **Learn.** Present the incident at the monthly platform guild; incorporate the lessons learned into [Time-Series Query Performance Governance](../standards/TIME_SERIES_QUERY_PERFORMANCE_GOVERNANCE.md) at the next 180-day review.

## Rollback

1. If the freeze causes a workload outage, switch the workload back to the previous time-series source (Prometheus, prior database, or pre-computed table).
