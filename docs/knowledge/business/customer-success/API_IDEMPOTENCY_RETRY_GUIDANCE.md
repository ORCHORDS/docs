...
4. **Establish authoritative state.** Collect method, resource, idempotency key, request fingerprint, stored response, and outcome. Prefer protocol responses, audit events, reproducible queries, signed artifacts, and machine-readable output to screenshots. Preserve raw values before normalization.
