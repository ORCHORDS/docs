# Account Recovery Channel Change



Account Recovery Channel Change governs customer-success work involving recovery email, phone, or authenticator changes. It coordinates a demonstrably correct customer outcome while leaving specialist decisions with the identity program owner. The case is complete only when authoritative state, customer-visible behavior, and the explanation given to the customer agree.
...
Open this workflow for any request, alert, or credible near miss concerning recovery email, phone, or authenticator changes. Record the customer or tenant, product, environment, requested outcome, urgency, possible harm, contractual commitments, and systems likely to hold evidence. Preserve the original narrative separately from observations and hypotheses. Routine troubleshooting may continue only when it cannot overwrite evidence, broaden access, or obstruct later verification.



This article does not replace law, contracts, or an approved technical runbook. The identity program owner must identify the applicable authority and procedure version. Customer success owns coordination, understandable updates, commitments, and closure evidence—not technical or legal determinations.
...
2. **Assign authority.** Name a coordinator, the identity program owner, an operator, and an independent verifier. Define who may inspect restricted information, approve action, and communicate externally.
...
4. **Establish authoritative state.** Collect old and new channels, existing authenticators, risk signals, delay, approvals, and dual-channel notices. Prefer protocol responses, audit events, reproducible queries, signed artifacts, and machine-readable output to screenshots. Preserve raw values before normalization.
...
The review packet contains intake, scope, authority, approvals, old and new channels, existing authenticators, risk signals, delay, approvals, and dual-channel notices, action identifiers, independent test output, delivery records, and closure rationale. It must distinguish evidence captured before action from evidence generated afterward. Reconcile expected and actual counts and investigate every unexplained difference. A green job status alone is not proof that the requested outcome occurred.
...
If the outcome cannot be proved, stop uncontrolled retries, preserve errors and correlation identifiers, and contain the most serious plausible impact. The identity program owner decides whether to retry through a controlled job, isolate a subsystem, roll back, or invoke incident management. Give the customer a factual checkpoint and next update time rather than an unsupported completion estimate.
