# Email Delivery Failure Triage

## Purpose and scope

Use this runbook for delayed or rejected mail. Its goal is to turn a customer symptom into safe, reproducible evidence and a bounded request to the team that owns the failing component. It does not replace engineering investigation, incident command, legal review, or a product service commitment.
...
Capture complete delivery-status notification, message ID, SMTP hop, enhanced status code. Preserve exact machine output instead of paraphrasing it. The topic-specific control is to separate transient 4xx conditions from persistent 5xx conditions. Never request passwords, private keys, session cookies, bearer credentials, payment data, or an unrestricted production dataset. Redact unrelated personal and proprietary material before attachment.
...
- [https://www.rfc-editor.org/rfc/rfc3463](https://www.rfc-editor.org/rfc/rfc3463)
