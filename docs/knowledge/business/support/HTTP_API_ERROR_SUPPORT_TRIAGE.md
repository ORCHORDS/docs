# HTTP API Error Support Triage

## Purpose and scope

Use this runbook for unexpected API behavior. Its goal is to turn a customer symptom into safe, reproducible evidence and a bounded request to the team that owns the failing component. It does not replace engineering investigation, incident command, legal review, or a product service commitment.
...
Capture method, sanitized URI, status, headers, correlation ID, body excerpt. Preserve exact machine output instead of paraphrasing it. The topic-specific control is to separate transport failure from a valid HTTP error response. Never request passwords, private keys, session cookies, bearer credentials, payment data, or an unrestricted production dataset. Redact unrelated personal and proprietary material before attachment.
