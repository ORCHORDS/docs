# OAuth Authorization Failure Triage

## Purpose and scope

Use this runbook for OAuth flow failure. Its goal is to turn a customer symptom into safe, reproducible evidence and a bounded request to the team that owns the failing component. It does not replace engineering investigation, incident command, legal review, or a product service commitment.
...
Capture flow stage, issuer, client ID, redirect URI, scopes, OAuth error. Preserve exact machine output instead of paraphrasing it. The topic-specific control is to never collect codes, tokens, secrets, or PKCE verifiers. Never request passwords, private keys, session cookies, bearer credentials, payment data, or an unrestricted production dataset. Redact unrelated personal and proprietary material before attachment.
