# Time Synchronization Support Diagnosis

## Purpose and scope

Use this runbook for clock disagreement. Its goal is to turn a customer symptom into safe, reproducible evidence and a bounded request to the team that owns the failing component. It does not replace engineering investigation, incident command, legal review, or a product service commitment.
...
Capture client and service UTC values, offset, time source, synchronization state, time zone. Preserve exact machine output instead of paraphrasing it. The topic-specific control is to retain original timestamps and the conversion method. Never request passwords, private keys, session cookies, bearer credentials, payment data, or an unrestricted production dataset. Redact unrelated personal and proprietary material before attachment.
...
- [https://www.rfc-editor.org/rfc/rfc5905](https://www.rfc-editor.org/rfc/rfc5905)
- [https://www.nist.gov/pml/time-and-frequency-division/time-services](https://www.nist.gov/pml/time-and-frequency-division/time-services)
