# IPv6 Connectivity Support Triage

## Purpose and scope

Use this runbook for dual-stack connectivity. Its goal is to turn a customer symptom into safe, reproducible evidence and a bounded request to the team that owns the failing component. It does not replace engineering investigation, incident command, legal review, or a product service commitment.
...
Capture A and AAAA answers, selected address, interface, route, packet-size symptom. Preserve exact machine output instead of paraphrasing it. The topic-specific control is to compare families without permanently disabling IPv6. Never request passwords, private keys, session cookies, bearer credentials, payment data, or an unrestricted production dataset. Redact unrelated personal and proprietary material before attachment.
