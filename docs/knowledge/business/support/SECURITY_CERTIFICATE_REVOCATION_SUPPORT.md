...
The exact sequence is to confirm serial and fingerprint, inventory deployments, revoke through the issuing authority, generate a new key when compromise is possible, deploy replacement chains, remove old material, query revocation publication, and test clients. Make one authorized reversible change at a time with an expected result, stop condition, and rollback. Verify the original operation and a nearby control; task acceptance alone is not recovery evidence.
...
- RFC 5280 X.509 Profile: https://www.rfc-editor.org/rfc/rfc5280
