# TLS Certificate Error Triage

## Purpose and scope

Use this runbook for TLS identity validation. Its goal is to turn a customer symptom into safe, reproducible evidence and a bounded request to the team that owns the failing component. It does not replace engineering investigation, incident command, legal review, or a product service commitment.
...
Capture hostname, port, chain fingerprints, validity, SANs, alert, interception state. Preserve exact machine output instead of paraphrasing it. The topic-specific control is to never recommend disabling verification. Never request passwords, private keys, session cookies, bearer credentials, payment data, or an unrestricted production dataset. Redact unrelated personal and proprietary material before attachment.
...
- [https://www.rfc-editor.org/rfc/rfc9525](https://www.rfc-editor.org/rfc/rfc9525)
