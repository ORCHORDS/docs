# Rate-Limit Support Handling

## Purpose and scope

Use this runbook for throttling responses. Its goal is to turn a customer symptom into safe, reproducible evidence and a bounded request to the team that owns the failing component. It does not replace engineering investigation, incident command, legal review, or a product service commitment.
...
Capture status, RateLimit and Retry-After fields, endpoint class, UTC window, request count. Preserve exact machine output instead of paraphrasing it. The topic-specific control is to distinguish quota, concurrency, burst, abuse control, and downstream protection. Never request passwords, private keys, session cookies, bearer credentials, payment data, or an unrestricted production dataset. Redact unrelated personal and proprietary material before attachment.
...
- [https://www.rfc-editor.org/rfc/rfc9333](https://www.rfc-editor.org/rfc/rfc9333)
