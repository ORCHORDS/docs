# Character Encoding Support Triage

## Purpose and scope

Use this runbook for corrupted character data. Its goal is to turn a customer symptom into safe, reproducible evidence and a bounded request to the team that owns the failing component. It does not replace engineering investigation, incident command, legal review, or a product service commitment.
...
Capture original bytes if lawful, media type, charset, source, expected code points, normalization. Preserve exact machine output instead of paraphrasing it. The topic-specific control is to avoid repeatedly saving already-corrupted data. Never request passwords, private keys, session cookies, bearer credentials, payment data, or an unrestricted production dataset. Redact unrelated personal and proprietary material before attachment.
