# CSV Import Failure Support

## Purpose and scope

Use this runbook for CSV import failure. Its goal is to turn a customer symptom into safe, reproducible evidence and a bounded request to the team that owns the failing component. It does not replace engineering investigation, incident command, legal review, or a product service commitment.
...
Capture sanitized reproducer, delimiter, quotes, line endings, encoding, header, row, locale. Preserve exact machine output instead of paraphrasing it. The topic-specific control is to treat spreadsheet formulas as potentially active content. Never request passwords, private keys, session cookies, bearer credentials, payment data, or an unrestricted production dataset. Redact unrelated personal and proprietary material before attachment.
