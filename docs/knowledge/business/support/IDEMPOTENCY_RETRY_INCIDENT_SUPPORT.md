# Idempotency and Retry Incident Support

## Purpose and scope

Use this runbook for possible duplicate side effects. Its goal is to turn a customer symptom into safe, reproducible evidence and a bounded request to the team that owns the failing component. It does not replace engineering investigation, incident command, legal review, or a product service commitment.
...
Capture operation, idempotency-key fingerprint, request times, retry count, resource IDs. Preserve exact machine output instead of paraphrasing it. The topic-specific control is to do not replay a state-changing request until its contract is confirmed. Never request passwords, private keys, session cookies, bearer credentials, payment data, or an unrestricted production dataset. Redact unrelated personal and proprietary material before attachment.
