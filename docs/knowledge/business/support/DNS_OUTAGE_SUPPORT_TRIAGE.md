# DNS Outage Support Triage

## Purpose and scope

Use this runbook for name resolution. Its goal is to turn a customer symptom into safe, reproducible evidence and a bounded request to the team that owns the failing component. It does not replace engineering investigation, incident command, legal review, or a product service commitment.
...
Capture queried name, record type, resolver, response code, flags, answer, authority, TTL, DNSSEC state. Preserve exact machine output instead of paraphrasing it. The topic-specific control is to distinguish NXDOMAIN, NODATA, timeout, delegation, and stale cache. Never request passwords, private keys, session cookies, bearer credentials, payment data, or an unrestricted production dataset. Redact unrelated personal and proprietary material before attachment.
...
- [https://www.rfc-editor.org/rfc/rfc2308](https://www.rfc-editor.org/rfc/rfc2308)
