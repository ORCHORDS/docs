# Web Accessibility Bug Intake

## Purpose and scope

Use this runbook for an accessibility barrier. Its goal is to turn a customer symptom into safe, reproducible evidence and a bounded request to the team that owns the failing component. It does not replace engineering investigation, incident command, legal review, or a product service commitment.
...
Capture URL, task, browser, assistive technology if volunteered, input method, expected and actual result. Preserve exact machine output instead of paraphrasing it. The topic-specific control is to offer an effective alternate route and do not demand a diagnosis. Never request passwords, private keys, session cookies, bearer credentials, payment data, or an unrestricted production dataset. Redact unrelated personal and proprietary material before attachment.
