---
title: "OpenID Connect CIBA Core 1.0"
...
The client initiates at the backchannel authentication endpoint with scope, client_notification_token for ping or push, binding_message, user_code and requested_expiry as applicable. A successful response returns auth_req_id, expires_in and optional interval. Poll mode receives authorization_pending, slow_down, access_denied or expired_token at the token endpoint; ping signals readiness and still requires token retrieval; push delivers tokens to the registered endpoint. Validate client authentication, login_hint/token/id_token_hint rules, delivery mode, signed authentication request when configured, user consent and binding-message display. Never treat a ping as authorization.
...
- [CIBA Core 1.0](https://openid.net/specs/openid-client-initiated-backchannel-authentication-core-1_0-final.html)
