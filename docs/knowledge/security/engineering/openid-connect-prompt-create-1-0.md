---
title: "Initiating User Registration via OpenID Connect"
...
The `create` value extends `prompt`. An authorization request with `prompt=create` asks the OpenID Provider to begin registration. Prompt values are space-delimited; because mutually exclusive conditions can arise, `create` should not be combined with other prompt values. The OP advertises prompt_values_supported containing create. The request is still an OIDC authorization request: validate client_id, redirect_uri, response_type, scope, state and nonce, and return standard authorization errors. Registration completion must not bypass authentication, consent, account policy or redirect validation.
...
- [Initiating User Registration via OpenID Connect](https://openid.net/specs/openid-connect-prompt-create-1_0-final.html)
