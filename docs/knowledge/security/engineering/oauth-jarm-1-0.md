---
title: "JWT Secured Authorization Response Mode (JARM)"
...
JARM defines a deliberately narrow extension point: the authorization response is a signed, optionally encrypted JWT in response using a jwt response mode and contains iss, aud, exp plus success or error parameters. Implementers must validate JOSE, issuer, client audience, expiry, algorithms and keys before state, code or error processing. Transport security is necessary but does not replace message-level issuer, audience, key, lifetime, purpose and replay validation. A syntactically valid object is not authorized merely because a parser accepts it.
