---
title: "Financial-grade API Part 2: Advanced Security Profile"
...
Advanced requires signed Request Objects, confidential clients authenticated by private_key_jwt or mutual TLS, protected authorization responses under its selected option, and mTLS sender-constrained access tokens. Validate every JWT signature, asymmetric algorithm, issuer, audience, expiry, nonce and registered key. Bind codes to client and exact redirect URI; consume them once. Resource servers compare the presented certificate with token confirmation.
...
- [Canonical specification](https://openid.net/specs/openid-financial-api-part-2-1_0-final.html)
