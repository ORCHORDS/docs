...
The RP redirects the user agent to end_session_endpoint with id_token_hint, logout_hint, client_id, post_logout_redirect_uri, state and ui_locales as applicable. The OP validates that a post-logout URI is registered for the identified client; state is returned only when redirecting. id_token_hint helps identify the RP and session, including an expired token when audience/session checks remain valid. Logout may require user confirmation, especially when hints are absent or ambiguous. Front-channel navigation is not proof that every downstream session or token was revoked.
