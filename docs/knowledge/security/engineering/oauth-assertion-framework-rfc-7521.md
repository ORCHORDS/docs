---
title: "OAuth Assertion Framework (RFC 7521)"
...
RFC 7521 defines a deliberately narrow extension point: grant assertions use grant_type plus assertion; client authentication uses client_assertion_type plus client_assertion. Implementers must validate format, issuer, subject, audience, time, authenticity, authorization and replay; use invalid_grant for grant failure and invalid_client for authentication failure. Transport security is necessary but does not replace message-level issuer, audience, key, lifetime, purpose and replay validation. A syntactically valid object is not authorized merely because a parser accepts it.
