...
RFC 7009 defines a deliberately narrow extension point: the TLS endpoint accepts form token and optional token_type_hint; invalid and successfully revoked tokens both receive HTTP 200. Implementers must authenticate confidential clients, treat the hint only as optimization, prevent logs, and propagate revocation through caches and related tokens. Transport security is necessary but does not replace message-level issuer, audience, key, lifetime, purpose and replay validation. A syntactically valid object is not authorized merely because a parser accepts it.
