---
title: "JWT Proof-of-Possession Confirmation (RFC 7800)"
...
# JWT Proof-of-Possession Confirmation (RFC 7800)

## Wire contract and normative rules

RFC 7800 defines a deliberately narrow extension point: cnf identifies a proof key; jwk embeds a public JWK, jwe carries an encrypted symmetric key, and kid identifies a key by application convention. Implementers must validate the enclosing JWT, resolve only trusted keys, and verify a fresh application-protocol proof; an unknown confirmation method is not bearer authority. Transport security is necessary but does not replace message-level issuer, audience, key, lifetime, purpose and replay validation. A syntactically valid object is not authorized merely because a parser accepts it.
