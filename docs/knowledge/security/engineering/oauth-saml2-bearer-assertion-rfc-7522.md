---
title: "SAML 2.0 Bearer Assertions for OAuth (RFC 7522)"
...
# SAML 2.0 Bearer Assertions for OAuth (RFC 7522)

## Wire contract and normative rules

RFC 7522 defines a deliberately narrow extension point: a base64url SAML assertion is used as grant or client assertion; bearer SubjectConfirmation recipient and AudienceRestriction bind it to the token service. Implementers must validate trusted signature, subject, conditions, recipient, audience and replay; harden XML against XXE, wrapping and duplicate IDs. Transport security is necessary but does not replace message-level issuer, audience, key, lifetime, purpose and replay validation. A syntactically valid object is not authorized merely because a parser accepts it.
...
- [RFC 7522](https://www.rfc-editor.org/rfc/rfc7522.html)
