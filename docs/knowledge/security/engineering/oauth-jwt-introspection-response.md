...
RFC 9701 defines a deliberately narrow extension point: application/token-introspection+jwt carries the introspection response as a signed JWT, including active and issuer/audience context. Implementers must authenticate the caller and validate signature, issuer, resource-server audience, times, media type and key before trusting active. Transport security is necessary but does not replace message-level issuer, audience, key, lifetime, purpose and replay validation. A syntactically valid object is not authorized merely because a parser accepts it.
