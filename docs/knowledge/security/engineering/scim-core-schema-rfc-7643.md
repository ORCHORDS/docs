---
title: "SCIM Core Schema (RFC 7643)"
...
Every SCIM resource has schemas, and core User and Group attributes have defined type, multiValued, mutability, returned, uniqueness, caseExact and canonical-value characteristics. Common attributes include id, externalId and meta; id is service-provider issued, stable and non-reassignable within its namespace. Password is writeOnly and must never be returned. Extensions use their schema URI as a JSON attribute and include that URI in schemas. Unknown attributes and mutability rules must follow SCIM protocol processing rather than being silently stored as trusted data.
